        ...
        {minimumServices.map((service, index) => (
          ...
          >
            <div className="flex items-center gap-4">
              <h2 className={`font-bold ${clickedService === index ? 'text-white' : 'text-purple-700'} group-hover:text-white`}>
               0{index+1}
              </h2>
            ...
            <GoArrowUpRight className={`w-6 h-6 transition-transform duration-300 transform rotate-90 ${clickedService === index ? 'transform rotate-0 text-white' : 'text-purple-700'} text-purple-700`} />
          </div>
        ))}
      </div>
    </div>
