...
import AddAbout from "./sub-components/AddAbout";
import AddAwardRecognition from "./sub-components/AddAwardRecognition";
...
const HomePage = () => {
  ...
  return (
    <div className="flex min-h-screen w-full flex-col bg-indigo-400 bg-muted/40">
        ...
        <nav className="flex flex-col items-center gap-4 px-2 sm:py-5">
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Link
                  className={`flex h-9 w-9 items-center justify-center rounded-lg ${active === "Add About"
                    ? "text-accent-foreground bg-accent"
                    : "text-muted-foreground"
                    }  transition-colors hover:text-foreground md:h-8 md:w-8`}
                  onClick={() => setActive("Add About")}
                >
                  <Info className="h-5 w-5" />
                  <span className="sr-only">Add About</span>
                </Link>
              </TooltipTrigger>
              <TooltipContent side="right">Add About</TooltipContent>
            </Tooltip>
          </TooltipProvider>
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Link
                  className={`flex h-9 w-9 items-center justify-center rounded-lg ${active === "Add AwardRecognition"
                    ? "text-accent-foreground bg-accent"
                    : "text-muted-foreground"
                    }  transition-colors hover:text-foreground md:h-8 md:w-8`}
                  onClick={() => setActive("Add AwardRecognition")}
                >
                  <Medal className="h-5 w-5" />
                  <span className="sr-only">Add AwardRecognition</span>
                </Link>
              </TooltipTrigger>
              <TooltipContent side="right">Add Award & Recognition</TooltipContent>
            </Tooltip>
          </TooltipProvider>
        ...
        <Sheet>
          ...
          <SheetContent side="left" className="sm:max-w-xs">
            <nav className="grid gap-6 text-lg font-medium">
              ...
              {/* ////////////About/////////// */}
              <Link
                className={`flex items-center gap-4 px-2.5 ${active === "Add About"
                  ? "text-foreground"
                  : "text-muted-foreground hover:text-foreground "
                  }`}
                onClick={() => setActive("Add About")}
              ...
              {/* ///////////AwardRecognition//////////// */}
              <Link
                className={`flex items-center gap-4 px-2.5 ${active === "Add AwardRecognition"
                  ? "text-foreground"
                  : "text-muted-foreground hover:text-foreground "
                  }`}
                onClick={() => setActive("Add AwardRecognition")}
      ...
      {(() => {
        switch (active) {
          ...
          case "Add AwardRecognition":
            return <AddAwardRecognition />;
            break;
